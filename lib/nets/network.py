...
class Network(nn.Module):
  ...
  def _predict_test(self, ss_boxes):
    ...
    # flag = torch.zeros((1, 2, 2)).cuda()
    # flag[:, 0, 0] = torch.cos((b[:, 0, 0] * np.pi * 100) / 180)
    # flag[:, 0, 1] = torch.sin((b[:, 0, 1] * np.pi * 100) / 180)
    # flag[:, 1, 0] = torch.sin((b[:, 1, 0] * np.pi * -100) / 180)
    # flag[:, 1, 1] = torch.cos((b[:, 1, 1] * np.pi * 100) / 180)
  ...
  def _predict(self, ss_boxes, ss_boxes_1, ss_boxes_2):
    ...
    theta = torch.cat((1 / (b + 0.2), a), dim=2)
    grid = F.affine_grid(theta, att_ffe_net_conv_0.size())
    trans_x_1 = F.grid_sample(att_ffe_net_conv_0, grid)
    # ********************************RSD**********************************
    ...
    trans_x_2 = F.grid_sample(att_ffe_net_conv_1, grid)
    # ********************************RSD**********************************
    ...
    trans_x_3 = F.grid_sample(att_ffe_net_conv_2, grid)
    # ********************************RSD**********************************

    trans_x_1 = torch.sum(trans_x_1, dim=0, keepdim=True)
    trans_x_2 = torch.sum(trans_x_2, dim=0, keepdim=True)
    trans_x_3 = torch.sum(trans_x_3, dim=0, keepdim=True)

    net_conv_0 = att_net_conv_0 + trans_x_1
    net_conv_1 = att_net_conv_1 + trans_x_2
    net_conv_2 = att_net_conv_2 + trans_x_3
