...
from nets.OICR import OICR, rotation_invariant_graph_activate, smooth_l1_loss
...
class Network(nn.Module):
    def __init__(self):
        ...
        # self.FFE_2 = FFE(512)
        # self.FFE_3 = FFE(512)
        self.att1 = SpatialAttention(3)
        # self.att1_2 = SpatialAttention(3)
        # self.att1_3 = SpatialAttention(3)
        self.att2 = SpatialAttention(3)
        # self.att2_2 = SpatialAttention(3)
        # self.att2_3 = SpatialAttention(3)

        # self.relation1 = roi_relation(4096)
        # self.relation2 = roi_relation(1024)
    ...
    def _add_losses(self, sigma_rpn=3.0):
        ...
        roi_labels, roi_weights, keep_inds = self.get_refine_supervision(det_cls_product,
                                                                         self._image_gt_summaries['ss_boxes'][
                                                                         self.ss_boxes_indexes, :],
        ...
        roi_labels, roi_weights, keep_inds = self.get_refine_supervision(refine_prob_1,
                                                                         self._image_gt_summaries['ss_boxes'][
                                                                         self.ss_boxes_indexes, :],
                                                                         self._image_gt_summaries['image_level_label'])
        ...
        roi_labels, roi_weights, keep_inds = self.get_refine_supervision(refine_prob_2,
                                                                         self._image_gt_summaries['ss_boxes'][
                                                                         self.ss_boxes_indexes, :],
                                                                         self._image_gt_summaries['image_level_label'])
        ...
        # M_score = torch.cat((refine_prob_1, refine_prob_2, refine_prob_3), dim=1)

        roi_labels, roi_weights, keep_inds, bbox_targets, bbox_inside_weights, bbox_outside_weights, cls_weights = rotation_invariant_graph_activate(
            self, refine_prob_1, refine_prob_2, \
            refine_prob_3, self._image_gt_summaries['ss_boxes'][self.ss_boxes_indexes, :], \
            self._image_gt_summaries['ss_boxes'][self.ss_boxes_indexes, :], \
            self._image_gt_summaries['ss_boxes'][self.ss_boxes_indexes, :], \
            self._image_gt_summaries['image_level_label'])

        # keep_inds1 = keep_inds + len(self._image_gt_summaries['ss_boxes'][self.ss_boxes_indexes, :])
        # keep_inds2 = keep_inds1 + len(self._image_gt_summaries['ss_boxes'][self.ss_boxes_indexes, :])

        roi_weights = torch.tensor(roi_weights).cuda()
        # keep = np.concatenate([keep_inds, keep_inds1, keep_inds2])
        # roi_labels1 = np.concatenate([roi_labels, roi_labels, roi_labels])
        roi_labels = torch.tensor(roi_labels, dtype=roi_weights.dtype).cuda()
    ...
    '''
        ...
        for i in range(self._num_classes):
            if image_level_label[0, i] == 1:
                ...
                max_box_score = np.concatenate(
                    (max_box_score, cls_prob_tmp[max_index] * np.ones((1, 1), dtype=np.float32)), axis=0)
    ...
    def _region_classification(self, fc7_roi, fc7_context, fc7_frame, fc7_roi_1=None, fc7_roi_2=None):
        ...
        det_cls_prob = torch.sum(det_cls_prob_product,
                                 0)  # 1 x class_num or just a one dim vector whose size is class_num
    ...
    def _predict_test(self, ss_boxes):
        ...
        att_net_conv_0 = self.att1(net_conv) * net_conv

       

        # ****************************************************************
        ffe_blob_conv, a, b = self.FFE(net_conv)

        att_ffe_net_conv_0 = self.att2(ffe_blob_conv) * ffe_blob_conv

        theta = torch.cat((1 / (b + 0.2), a), dim=2)
        grid = F.affine_grid(theta, att_ffe_net_conv_0.size())
        trans_x = F.grid_sample(att_ffe_net_conv_0, grid)
        # ****************************************************************

        trans_x = torch.sum(trans_x, dim=0, keepdim=True)

        net_conv = att_net_conv_0 + trans_x

        # build the anchors for the image
        # self._anchor_component(net_conv.size(2), net_conv.size(3))

        # rois = self._region_proposal(net_conv)pytho
    ...
    def _predict(self, ss_boxes, ss_boxes_1, ss_boxes_2):
        ...
        att_net_conv_0 = self.att1(net_conv_0) * net_conv_0
        att_net_conv_1 = self.att1_2(net_conv_1) * net_conv_1
        att_net_conv_2 = self.att1_3(net_conv_2) * net_conv_2

        # ****************************************************************
        ffe_blob_conv, a, b = self.FFE(net_conv_0)

        att_ffe_net_conv_0 = self.att2(ffe_blob_conv) * ffe_blob_conv

        theta = torch.cat((1 / (b + 0.2), a), dim=2)
        grid = F.affine_grid(theta, att_ffe_net_conv_0.size())
        trans_x_0 = F.grid_sample(att_ffe_net_conv_0, grid)
        # ****************************************************************

        # ****************************************************************

        ffe_blob_conv, a, b = self.FFE_2(net_conv_1)

        att_ffe_net_conv_1 = self.att2_2(ffe_blob_conv) * ffe_blob_conv

        theta = torch.cat((1 / (b + 0.2), a), dim=2)
        grid = F.affine_grid(theta, att_ffe_net_conv_1.size())
        trans_x_1 = F.grid_sample(att_ffe_net_conv_1, grid)
        # ****************************************************************

        # ****************************************************************

        ffe_blob_conv, a, b = self.FFE_3(net_conv_2)

        att_ffe_net_conv_2 = self.att2_3(ffe_blob_conv) * ffe_blob_conv

        theta = torch.cat((1 / (b + 0.2), a), dim=2)
        grid = F.affine_grid(theta, att_ffe_net_conv_2.size())
        trans_x_2 = F.grid_sample(att_ffe_net_conv_2, grid)
        # ****************************************************************

        trans_x_0 = torch.sum(trans_x_0, dim=0, keepdim=True)
        trans_x_1 = torch.sum(trans_x_1, dim=0, keepdim=True)
        trans_x_2 = torch.sum(trans_x_2, dim=0, keepdim=True)

        net_conv_0 = att_net_conv_0 + trans_x_0
        net_conv_1 = att_net_conv_1 + trans_x_1
        net_conv_2 = att_net_conv_2 + trans_x_2
        ...
        # print('pool5 ', pool5.shape)
        fc7_roi = self._head_to_tail(pool5_roi)
        # fc7_roi = self.relation1(fc7_roi, rois, None, None, False, nongt_dim=300)
        # fc7_roi = self.relation2(fc7_roi, rois, None, None, False, nongt_dim=300)
        # fc7_roi = self.linear1(fc7_roi)
    ...
    def forward(self, image, image1, image2, image_level_label, im_info, gt_boxes=None, ss_boxes=None,
                ss_boxes_rotate_1=None, ss_boxes_rotate_2=None, mode='TRAIN'):
        ...
        if mode == 'TRAIN':
            rois, cls_prob, det_prob, bbox_pred, cls_det_prob_product, det_cls_prob = self._predict(
                ss_boxes[self.ss_boxes_indexes, :], \
                ss_boxes_rotate_1[self.ss_boxes_indexes, :], \
                ss_boxes_rotate_2[self.ss_boxes_indexes, :])
        else:
            rois, cls_prob, det_prob, bbox_pred, cls_det_prob_product, det_cls_prob = self._predict_test(
                ss_boxes[self.ss_boxes_indexes, :])
    ...
    def test_image(self, image, im_info, ss_boxes):
        ...
        bbox_pred, rois, det_cls_prob, det_cls_prob_product, refine_prob_1, refine_prob_2, refine_prob_3, rotate_score = \
        ...
        return bbox_pred, rois, det_cls_prob, det_cls_prob_product, refine_prob_1[:, 1:], refine_prob_2[:,
                                                                                          1:], refine_prob_3[:,
                                                                                               1:], rotate_score[:, 1:]
    ...
    def get_summary(self, blobs):
        self.eval()
        self.forward(blobs['data'], blobs['data1'], blobs['data2'], blobs['image_level_labels'], blobs['im_info'],
                     blobs['gt_boxes'], blobs['ss_boxes'], blobs['ss_boxes_rotate1'], blobs['ss_boxes_rotate2'])
        self.train()
    ...
    def train_step(self, blobs, train_op):
        ...
        self.forward(blobs['data'], blobs['data1'], blobs['data2'], blobs['image_level_labels'], blobs['im_info'],
                     blobs['gt_boxes'], blobs['ss_boxes'], blobs['ss_boxes_rotate1'], blobs['ss_boxes_rotate2'])
        cls_det_loss, refine_loss_1, refine_loss_2, refine_loss_3, rotate_loss, loss_box, loss = self._losses[
                                                                                                     'cls_det_loss'].item(), \
                                                                                                 self._losses[
                                                                                                     'refine_loss_1'].item(), \
                                                                                                 self._losses[
 ...
 \
    ...
    def train_step_no_return(self, blobs, train_op):
        self.forward(blobs['data'], blobs['data1'], blobs['data2'], blobs['im_info'], blobs['gt_boxes'],
                     blobs['ss_boxes'])
        train_op.zero_grad()
        self._losses['total_loss'].backward()
        train_op.step()
